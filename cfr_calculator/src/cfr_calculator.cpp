...
namespace cfr_calculator {
  ...
  bool CFRCalculator::computeCFR(const std::vector<std::shared_ptr<primitive_motion_level_tools::PrimitiveState> >& supportEEFs,
    ...
    if(!static_equilibuim_test::calcProjection(A,b,C,dl,du,this->M_,this->l_,this->u_,this->vertices_,debugLevel, 0.05,30,false,1e-15)){ // toleranceは0.05より小さいと時間がかかりすぎる. lptoleranceは1e-15くらい小さくないと，vertexを見逃す
